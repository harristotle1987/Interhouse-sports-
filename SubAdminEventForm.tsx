...
/**
 * SUB-ADMIN EVENT PROVISIONING FORM [V10.1 - SIGNAL-FREE]
 * Removed AbortController to prevent STAGING_FAILURE in high-speed environments.
 */
const SubAdminEventForm: React.FC<{ onEventCreated: (event: any) => void, admin: AdminUser }> = ({ onEventCreated, admin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("STAGING_ERROR_CAUGHT:", err);
      const localEvent = { ...payload, id: `local-${Date.now()}` } as LiveMatch;
      addLocalEvent(localEvent);
      setStatus({ type: 'offline', message: 'LOCAL_BUFFER_ACTIVE: Record saved locally.' });
    } finally {
  ...
  return (
    <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-2xl max-w-3xl mx-auto relative overflow-hidden animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="flex items-center gap-6 mb-10 pb-8 border-b border-slate-50 text-left">
        <div className="p-4 bg-indigo-50 text-indigo-600 rounded-[1.25rem]"><Zap size={28} /></div>
        <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
        <div className="space-y-8 text-left">
          ...
          <input required value={name} onChange={e => setName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 px-8 py-6 text-slate-900 font-black text-2xl rounded-3xl focus:outline-none focus:border-indigo-600 transition-all uppercase italic" />
          <div className="grid grid-cols-2 gap-8">
             <select value={category} onChange={e => setCategory(e.target.value as EventCategory)} className="w-full bg-slate-50 border border-slate-200 px-6 py-5 text-slate-900 font-black rounded-2xl appearance-none uppercase italic text-sm">
                {Object.values(EventCategory).map(cat => <option key={cat} value={cat}>{cat}</option>)}
             </select>
             <select value={scoringType} onChange={e => setScoringType(e.target.value as ScoringType)} className="w-full bg-slate-50 border border-slate-200 px-6 py-5 text-slate-900 font-black rounded-2xl appearance-none uppercase italic text-sm">
        ...
        {status && <div className="p-5 bg-emerald-50 text-emerald-700 text-xs font-black rounded-2xl">{status.message}</div>}
        <button type="submit" disabled={isSubmitting} className="w-full py-9 bg-slate-900 text-white font-black uppercase italic tracking-[0.7em] rounded-[2rem] hover:bg-indigo-600 transition-all flex items-center justify-center gap-5 shadow-2xl active:scale-[0.98] disabled:opacity-50 text-lg">
