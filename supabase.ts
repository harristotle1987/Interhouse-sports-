...
/**
 * SOVEREIGN AUTH SINGLETON [V10.5 - HARDENED]
 * Ensures exactly one GoTrueClient instance exists in hardware memory.
 * Optimized for PWA standalone mode with dedicated storage keys.
 */
  ...
  if (!global._supabaseInstance) {
    global._supabaseInstance = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
      auth: {
        ...
        storageKey: 'sovereign-vault-token'
      }
    });
  }
