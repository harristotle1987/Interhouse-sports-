...
const AdminProvisioning: React.FC = () => {
  ...
  const handleProvision = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!supabaseAdmin) {
      setError("UPLINK_FAILURE: SERVICE_ROLE_KEY_NOT_FOUND");
      return;
    }
  ...
  return (
    <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.15)] relative overflow-hidden text-left animate-in fade-in slide-in-from-bottom-8">
      ...
      <div className="flex items-center gap-6 mb-12 pb-10 border-b border-slate-50">
        <div className="p-5 bg-zinc-900 text-white rounded-[1.5rem] shadow-xl">
          <UserPlus size={32} strokeWidth={2.5} />
        </div>
        <div className="flex flex-col text-left">
          <h2 className="text-4xl font-black text-slate-900 uppercase italic tracking-tighter leading-none">High Command Access</h2>
          <span className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.4em] italic mt-3 flex items-center gap-2">
            <ShieldCheck size={12} /> ARCHITECT_AUTHORITY_ACTIVE
          </span>
        </div>
      </div>
      ...
      <form onSubmit={handleProvision} className="space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-left">
          <div className="space-y-3">
            ...
            <div className="relative">
              <select value={formData.role} onChange={e => setFormData({...formData, role: e.target.value as AdminRole})} className="w-full bg-slate-50 border border-slate-200 px-8 py-5 text-slate-900 font-black rounded-2xl focus:outline-none focus:border-zinc-600 transition-all appearance-none cursor-pointer text-sm italic uppercase">
                <option value={AdminRole.SUB_ADMIN}>SUB_ADMIN (OFFICIAL)</option>
                <option value={AdminRole.SUPER_KING}>SUPER_KING (ARCHITECT)</option>
                <option value={AdminRole.MEMBER}>MEMBER (OPERATIVE)</option>
              </select>
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              <select value={formData.arm} onChange={e => setFormData({...formData, arm: e.target.value as SchoolArm})} className="w-full bg-slate-50 border border-slate-200 px-10 py-5 text-slate-900 font-black rounded-2xl focus:outline-none focus:border-zinc-600 appearance-none cursor-pointer text-sm italic uppercase">
        ...
        <button type="submit" disabled={loading} className="w-full py-8 bg-zinc-900 text-white font-black uppercase italic tracking-[0.6em] rounded-3xl hover:bg-emerald-600 hover:text-black transition-all flex items-center justify-center gap-5 shadow-2xl active:scale-[0.98] disabled:opacity-50 text-lg">
          {loading ? <Loader2 className="animate-spin" size={28} /> : <>Commit Identity <Fingerprint size={24} /></>}
        </button>
      </form>
    </div>
  );
